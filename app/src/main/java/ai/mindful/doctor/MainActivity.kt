...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener,
    ...
    private fun saveLocation() {
        ...
        fusedLocationProviderClient.lastLocation.addOnCompleteListener {
            if (it.isSuccessful){
                prefs[SharedPrefKeys.LOCATION_LON.toString()] = it.result.longitude.toString()
                prefs[SharedPrefKeys.LOCATION_LAN.toString()] = it.result.latitude.toString()
                Log.i("location updated", "success")
            } else {
                Log.e("location updated", "failed: ${it.exception?.localizedMessage}")
            }
        }
    }
        ...
        when (requestCode) {
            1 -> {
                if (grantResults.isNotEmpty() && grantResults[0] ==
                    ...
                    Toast.makeText(this, "Location is required to connect you to appropriate patients!", Toast.LENGTH_SHORT).show()
                }
                return
            }
        }
